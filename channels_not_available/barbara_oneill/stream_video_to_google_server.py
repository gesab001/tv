...
def streamToGoogleCloudServer(jsonobj):
      formats = jsonobj["formats"]
      totalformats = len(formats)
      formattype = 13
      formatobj = formats[formattype]
      print(formatobj)
      url = formatobj["url"]
      command = 'ffmpeg -re -i ' +  '"'+url+'"' + ' -vcodec libx264 -vprofile baseline -g 30 -acodec aac -strict -2  -preset fast -b:v 5M -maxrate 6M -bufsize 3M -threads 4 -f flv rtmp://10.128.0.5:1935/show/stream'
      subprocess.call(command, shell=True)
