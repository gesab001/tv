...
parser.add_argument('--port', dest='port', type=str, help='Add port')
...
def streamToWindowServer(jsonobj):      
      formats = jsonobj["formats"]
      totalformats = len(formats)
      formattype = 13
      formatobj = formats[formattype]
...
def streamToUDP(jsonobj):      
      ...
      formatobj = formats[formattype]
      url = formatobj["url"]
      command = 'ffmpeg -re -i ' +  '"'+url+'"' + ' -vcodec libx264 -vprofile baseline -g 30 -acodec aac -strict -2  -preset fast -b:v 5M -maxrate 6M -bufsize 3M -threads 4 -f flv udp://localhost:' + port
      command = 'ffmpeg -i ' + '"' + url + '"' + ' -v 0 -vcodec mpeg4 -f mpegts udp://localhost:' + port
      print(command)
      proceed = input("continue")
      subprocess.call(command, shell=True)
